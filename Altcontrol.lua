...
local OPS_SAVE_KEY = "RobotSavedOperators_v4"
...
local function processNotifQueue()
	...
	titleLbl.Size = UDim2.new(1,-18,0,18) titleLbl.Position = UDim2.new(0,10,0,4)
	...
	msgLbl.Size = UDim2.new(1,-18,0,24) msgLbl.Position = UDim2.new(0,10,0,22)
	msgLbl.BackgroundTransparency = 1 msgLbl.Text = data.message
	msgLbl.TextColor3 = Color3.fromRGB(170,170,200) msgLbl.TextScaled = true msgLbl.Font = Enum.Font.Gotham
	msgLbl.TextXAlignment = Enum.TextXAlignment.Left msgLbl.TextWrapped = true msgLbl.Parent = frame
...
local function showConnectionRequest(requesterName)
	...
	local frame = Instance.new("Frame")
	frame.Size = UDim2.new(0,290,0,86)
	...
	TweenService:Create(frame, TweenInfo.new(0.25,Enum.EasingStyle.Quart,Enum.EasingDirection.Out), {Position=UDim2.new(0,16,1,-102)}):Play()

	local titleLbl = Instance.new("TextLabel")
	titleLbl.Size = UDim2.new(1,-14,0,18) titleLbl.Position = UDim2.new(0,10,0,4)
	titleLbl.BackgroundTransparency = 1 titleLbl.Text = "Operator Request"
	titleLbl.TextColor3 = Color3.fromRGB(255,180,80) titleLbl.TextScaled = true titleLbl.Font = Enum.Font.GothamBold
	titleLbl.TextXAlignment = Enum.TextXAlignment.Left titleLbl.Parent = frame
	local msgLbl = Instance.new("TextLabel")
	msgLbl.Size = UDim2.new(1,-14,0,18) msgLbl.Position = UDim2.new(0,10,0,24)
	msgLbl.BackgroundTransparency = 1 msgLbl.Text = requesterName.." wants to control you"
	msgLbl.TextColor3 = Color3.fromRGB(200,200,230) msgLbl.TextScaled = true msgLbl.Font = Enum.Font.Gotham
	msgLbl.TextXAlignment = Enum.TextXAlignment.Left msgLbl.Parent = frame

	local acceptBtn = Instance.new("TextButton")
	acceptBtn.Size = UDim2.new(0,106,0,24) acceptBtn.Position = UDim2.new(0,10,0,56)
	...
	denyBtn.Size = UDim2.new(0,86,0,24) denyBtn.Position = UDim2.new(0,122,0,56)
...
local masterConnection = RunService.Heartbeat:Connect(function()
...
local function startMirror()
	...
	mirrorConnection = RunService.Heartbeat:Connect(function()
		...
		if opVel.Magnitude > 1 then
			myHum:MoveTo(myRoot.Position + Vector3.new(opVel.X,0,opVel.Z).Unit * 3)
		end
...
local function stopMirror()
	mirrorEnabled = false
	if mirrorConnection then mirrorConnection:Disconnect() mirrorConnection = nil end
	notify("Mirror", "Disabled.", 2, Color3.fromRGB(180,100,255))
end
...
local function followPlayer(targetName)
	...
	followConnection = RunService.Heartbeat:Connect(function()
		...
		if dist > 5 then
			mh:MoveTo(tr.Position - (tr.Position - mr.Position).Unit * 4)
		else
			mh:MoveTo(mr.Position)
		end
	end)
end
...
local function patrol(names)
	...
	for _, name in ipairs(names) do
		local p = findPlayer(name) if p then table.insert(targets, p) end
	end
	...
	followConnection = RunService.Heartbeat:Connect(function()
		...
		local dist = (mr.Position - tr.Position).Magnitude
		if dist <= 5 then
			cooldown = true index = (index % #targets) + 1
...
local function lookAt(targetName)
	local t = findPlayer(targetName) if not t or not t.Character then return end
	local mc = localPlayer.Character if not mc then return end
	local mr = mc:FindFirstChild("HumanoidRootPart")
	local tr = t.Character:FindFirstChild("HumanoidRootPart")
...
local function setSpin(enabled)
	spinEnabled = enabled
	if not enabled then
		notify("Spin", "Off", 1, Color3.fromRGB(200,200,255))
	end
end
...
local function bigHead(e)
	local char = localPlayer.Character if not char then return end
	local head = char:FindFirstChild("Head")
	if head then head.Size = e and Vector3.new(4,4,4) or Vector3.new(2,1,1) end
end

local function crouchMode(e)
	local char = localPlayer.Character if not char then return end
	local hum = char:FindFirstChildOfClass("Humanoid")
	if hum then hum.HipHeight = e and 0.5 or 2 end
end

local function setTransparency(n)
	local char = localPlayer.Character if not char then return end
	for _, part in ipairs(char:GetDescendants()) do
		if part:IsA("BasePart") or part:IsA("Decal") then part.Transparency = n end
	end
end
...
local function setSize(n)
	...
	pcall(function()
		hum.BodyDepthScale.Value=n hum.BodyHeightScale.Value=n
		hum.BodyWidthScale.Value=n hum.HeadScale.Value=n
	end)
end

local function antiAFK(e)
	...
	notify("Anti AFK", e and "Enabled." or "Disabled.", 2, e and Color3.fromRGB(100,255,180) or Color3.fromRGB(255,180,100))
end

local function setWalkAnim(e)
	local char = localPlayer.Character if not char then return end
	local animate = char:FindFirstChild("Animate")
	if animate then animate.Disabled = not e end
end

local function glitchEffect(e)
	...
	if e then
		glitchConnection = RunService.Heartbeat:Connect(function()
			local char = localPlayer.Character if not char then return end
			local root = char:FindFirstChild("HumanoidRootPart")
			if root then root.CFrame = root.CFrame * CFrame.new(math.random(-2,2)*0.08,0,math.random(-2,2)*0.08) end
		end)
	end
end

local function setHealth(n)
	local char = localPlayer.Character if not char then return end
	local hum = char:FindFirstChildOfClass("Humanoid")
	if hum then hum.Health = math.clamp(n,0,hum.MaxHealth) end
end

local function ragdoll(e)
	local char = localPlayer.Character if not char then return end
	local hum = char:FindFirstChildOfClass("Humanoid") if not hum then return end
	if e then hum:ChangeState(Enum.HumanoidStateType.Ragdoll)
...
local function headless(e)
	local char = localPlayer.Character if not char then return end
	local head = char:FindFirstChild("Head")
	local hrp = char:FindFirstChild("HumanoidRootPart")
	if head then head.Transparency = e and 1 or 0 end
	if hrp then hrp.Transparency = e and 1 or 0 end
end
...
local function emote(name)
	local g = game:GetService("TextChatService").TextChannels:FindFirstChild("RBXGeneral")
	if g then g:SendAsync("/e "..name) end
end
...
processCommand = function(message, speaker)
	...
	if controlLocked then
		...
		local fw = c2:match("%.(%S+)")
		if fw then
			local res = ALIASES[fw:lower()] or fw:lower()
			if res~="lockcontrol" and res~="lck" and res~="lcc" and res~="lockconn" then return end
		end
	end
	...
	elseif cmd=="say" then if rest~="" then sayChat(rest) end
	elseif cmd=="sit" then
		local char=localPlayer.Character if char then local h=char:FindFirstChildOfClass("Humanoid") if h then h.Sit=true end end
		notify("Sit","Sitting.",1,Color3.fromRGB(180,180,255))
	elseif cmd=="stand" then
		local char=localPlayer.Character if char then local h=char:FindFirstChildOfClass("Humanoid") if h then h.Sit=false end end
	...
	elseif cmd=="speed" then local n=tonumber(args[1]) if n then setSpeed(n) notify("Speed",tostring(n),2,Color3.fromRGB(100,220,255)) end
	elseif cmd=="jump" then doJump()
	elseif cmd=="jumppower" then local n=tonumber(args[1]) if n then setJump(n) notify("JumpPower",tostring(n),2,Color3.fromRGB(100,220,255)) end
	...
	elseif cmd=="tpme" then
		...
		if op and op.Character then
			...
			local mc=localPlayer.Character
			if mc and or2 then local mr=mc:FindFirstChild("HumanoidRootPart") if mr then mr.CFrame=or2.CFrame+Vector3.new(3,0,0) end end
		end
	...
	elseif cmd=="size" then local n=tonumber(args[1]) if n then setSize(n) notify("Size",tostring(n),2,Color3.fromRGB(200,200,255)) end
	elseif cmd=="savepos" then
		local slot=tonumber(args[1]) or 1
		local char=localPlayer.Character if not char then return end
		local root=char:FindFirstChild("HumanoidRootPart") if not root then return end
		savedPositions[slot]=root.CFrame
		notify("Saved","Slot "..slot.." saved.",2,Color3.fromRGB(100,200,255))
	...
	elseif cmd=="health" then
		local char=localPlayer.Character if char then local h=char:FindFirstChildOfClass("Humanoid") if h then notify("Health",math.floor(h.Health).."/"..math.floor(h.MaxHealth),3,Color3.fromRGB(255,100,100)) end end
	elseif cmd=="sethealth" then local n=tonumber(args[1]) if n then setHealth(n) notify("Health","Set to "..n,2,Color3.fromRGB(255,100,100)) end
	elseif cmd=="pos" then
		local char=localPlayer.Character if char then local r=char:FindFirstChild("HumanoidRootPart") if r then local p=r.Position notify("Position",math.floor(p.X).." "..math.floor(p.Y).." "..math.floor(p.Z),3,Color3.fromRGB(100,200,255)) end end
	elseif cmd=="rig" then
		local char=localPlayer.Character if char then local h=char:FindFirstChildOfClass("Humanoid") if h then notify("Rig",h.RigType==Enum.HumanoidRigType.R15 and "R15" or "R6",2,Color3.fromRGB(200,200,255)) end end
	...
	elseif cmd=="lcc" then
		if rest=="on" then connectionLocked=true notify("Conn Lock","Locked. No new operators.",3,Color3.fromRGB(255,80,80))
		elseif rest=="off" then connectionLocked=false notify("Conn Lock","Open.",3,Color3.fromRGB(80,255,120))
		...
		if rest~="" then
			for i,op in ipairs(OPERATORS) do
				if op:lower()==rest:lower() then
					...
					notify("Removed",rest.." removed.",2,Color3.fromRGB(255,180,80)) break
				end
			end
		end
	elseif cmd=="ops" then
	...
	elseif cmd=="status" then
		notify("Status",
			(following and "Following" or "Idle").." | "..(loopCmd and "Loop: "..loopCmd or "No loop").." | "..(controlLocked and "Ctrl LOCKED" or "Free").." | "..(connectionLocked and "Conn LOCKED" or "Conn Open"),
...
notify("Robot Ready","Ops: "..(#OPERATORS>0 and table.concat(OPERATORS,", ") or "None - type .c to connect"),5,Color3.fromRGB(100,200,255))
